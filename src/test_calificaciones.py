from calificaciones import *

# Ejercicio 1
    # Apartado a
notaAda = calcular_nota(30,23,3)

    # Apartado b y c
totalRespuestas = int(input("¿Cuántas preguntas tiene el test? "))
aciertos = int(input("¿Cuántas ha acertado? "))
errores = int(input("¿Cuántas ha fallado? "))
notaAda = calcular_nota(totalRespuestas,aciertos,errores)
print("Ada, quien ha acertado", aciertos, "y fallado", errores, "de las", totalRespuestas, "preguntas, ha sacado un", notaAda)


# Ejercicio 2
    # Apartado a
print(calcula_nota_cuatrimestre((7, 8 ,7), 10, 4.5))

    # Apartado b
print(calcula_nota_evaluacion_continua((7, 8 ,7, 6, 9, 10), (10, 9), (6, 7)))

    # Apartado c y d
c1 = float(input("¿Cuánto sacó en el primer cuestionario? "))
c2 = float(input("¿Y en el segundo? "))
c3 = float(input("¿Y en el tercero? "))
c4 = float(input("¿Y en el cuarto? "))
c5 = float(input("¿Y en el quinto? "))
c6 = float(input("¿Y en el sexto? "))
cuestionarios = (c1,c2,c3,c4,c5,c6)
pa1 = float(input("¿Cuánto sacó en el primer parcial? "))
pa2 = float(input("¿Y en el segundo? "))
parciales = (pa1, pa2)
pr1 = float(input("¿Cuánto sacó en el primer proyecto? "))
pr2 = float(input("¿Y en el segundo? "))
proyectos = (pr1, pr2)
notafinal = (calcula_nota_evaluacion_continua(cuestionarios, parciales, proyectos))
print("En ese caso, la nota que ha sacado Ada en la evaluación continua es", notafinal)
